        ...
        public static TextureAtlas FromFile(ContentManager content, string fileName)
        {
            ...
            if (animations != null)
            {
                foreach (var animation in animations)
                {
                    ...
                    TimeSpan delay = TimeSpan.FromMilliseconds(double.Parse(animation.Attribute("delay")?.Value));

                    var framesElements = animation.Elements("Frame");
                    var frames = new List<TextureRegion>();

                    foreach (var frame in framesElements)
                    {
